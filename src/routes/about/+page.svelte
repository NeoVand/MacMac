<script lang="ts">
	...
	function drawVisual() {
		...
		// Draw a nice trimodal distribution
		const pdf = (x: number) =>
		...
		g.addColorStop(0.5, 'rgba(120, 60, 230, 0.05)');
		...
		ctx.moveTo(pathPts[0][0], pathPts[0][1]);
		for (let i = 1; i < pathPts.length; i++) ctx.lineTo(pathPts[i][0], pathPts[i][1]);
		const sg = ctx.createLinearGradient(toX(xMin), 0, toX(xMax), 0);
		sg.addColorStop(0, '#00ccff');
		sg.addColorStop(0.5, '#a855f7');
		sg.addColorStop(1, '#00ccff');
		ctx.strokeStyle = sg;
		ctx.lineWidth = 2;
		ctx.stroke();

		// Sample dots scattered along the distribution
...
<div class="mx-auto min-h-dvh max-w-xl px-6 py-8 sm:px-10 sm:py-12">
	<!-- Logo back link -->
	...
	<div class="space-y-4 text-[13px] leading-relaxed text-white/40">
		...
		<h2 class="pt-1 text-sm font-semibold text-white/55">How it works</h2>

		<p>
			Each level shows a target probability density function. You click to place samples.
			As you add points, a
			...
			builds up to show what your samples imply about the underlying distribution.
		</p>

		<p>Your score is based on two factors:</p>

		<ul class="list-inside list-disc space-y-1 pl-1 text-white/35">
			<li>
				<strong class="text-white/50">Accuracy</strong> — measured by
			...
			<li>
				<strong class="text-white/50">Efficiency</strong> — fewer clicks means a higher score
			</li>
		</ul>
		...
		<h2 class="pt-1 text-sm font-semibold text-white/55">Why it matters</h2>
		...
		<div class="mt-6 flex flex-col gap-3 border-t border-white/5 pt-5">
			...
			<div class="flex gap-4">
				<a
					...
					target="_blank"
					rel="noopener"
					class="flex items-center gap-1.5 text-xs text-white/25 transition hover:text-white/55"
				>
